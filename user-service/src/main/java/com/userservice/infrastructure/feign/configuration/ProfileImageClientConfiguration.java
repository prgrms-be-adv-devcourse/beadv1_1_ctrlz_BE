...
@Configuration
public class ProfileImageClientConfiguration {

	@Bean(name = "profileImageFeignLoggerLevel")
	Logger.Level feignLoggerLevel() {
		/*
			헤더에 토큰을 담아서 요청을 보낼수도 있을거 같아서 헤더로 했습니다
			TODO:
			 FeignClient 사용 후 남는 로그 확인 후 Header가 불필요해지면 BASIC 수정 필요.
			 필요에 따라 CustomLogger 구현 후 공통된 Logger로 전환 하는 등 리팩토링 필요
		*/
		return Logger.Level.BASIC;
	}

	@Bean
	public ErrorDecoder profileImageClientErrorDecoder() {
