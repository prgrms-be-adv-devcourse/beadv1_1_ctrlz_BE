...
public class QdrantVectorRepository implements VectorRepository {
	...
	//  * @return 검색 결과 리스트
	//  */
	// public List<DocumentSearchResponse> hybridSearch(String query, int maxResults, double denseWeight) {
	// 	log.info("Qdrant Hybrid Search 시작: query='{}', maxResults={}, denseWeight={}",
	...
	// 	List<DocumentSearchResponse> denseResults = similaritySearch(query, maxResults);
	// 	log.info("Dense 검색 결과: {} 건", denseResults.size());
	//
	// 	// 2. Sparse Vector 생성 및 검색 (키워드 기반)
	// 	Map<Integer, Float> sparseVector = sparseEmbedder.generateSparseVector(query);
	// 	List<DocumentSearchResponse> sparseResults = performSparseSearch(sparseVector, maxResults * 2);
	...
	// 		hybridResults.size(), denseResults.size(), sparseResults.size(), fusedScores.size());
	//
	...
	//  */
	// private List<DocumentSearchResponse> performSparseSearch(Map<Integer, Float> querySparseVector, int maxResults) {
	...
	// 		"실제 Sparse Vector 지원을 위해서는 Qdrant Collection에 Named Sparse Vector 설정이 필요합니다.");
	//
